...
export const updateUser = async (req, res) => {
  const id = req.params.id;
  const { _id, password, firstname, lastname, username } = req.body;
  if (id === _id) {
    try {
      ...
      const user = await UserModel.findByIdAndUpdate(
        _id,
        { $set: { firstname: firstname, lastname: lastname, usrname: username } },
        { new: true }
      );
...
export const updateProfile = async (req, res) => {
  ...
  if (id === _id) {
    try {
      const mycloud = await cloudinary.v2.uploader.upload(profileImage, { folder: "Profile" });
      const user = await UserModel.findByIdAndUpdate(
        _id,
        { $set: { profileImage: { public_id: mycloud.public_id, url: mycloud.url } } },
        { new: true }
      );
      const token = jwt.sign({ username: user.username, id: user._id }, process.env.JWTKEY, {
        expiresIn: "1h",
